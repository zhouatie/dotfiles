...
> 如果 POPO文档(docType=1) 的 content 中包含图片或其他资源，需要提取 `src` 中的链接，按 [doc-resource-reference.md](./doc-resource-reference.md) 通过 `popo_doc_get_file_download_url` 获取临时下载地址后才能下载；团队空间文档必须同时传 `teamSpaceId`。多维表(docType=9)附件字段不适用本规则，必须走 `doc_file_batch_get`。
...
| 参数 | 说明 |
|------|------|
| `type` | 操作类型：`doc.replace_page`、`doc.replace_node`、`doc.insert_before`、`doc.insert_after` |
...
### ⚠️ `doc.replace_page` 必须用 `<doc>` 根节点包裹（必读）

`doc.replace_page` 用于全量替换整个 POPO 文档内容，服务端要求 `content` 为**单一 `<doc>` 根节点**，所有块级标签包在 `<doc>...</doc>` 内。漏包会报错：

```
Whole-document replacement requires a single top-level <doc> root. Wrap the full document content in <doc>...</doc>.
```

| 命令 | docType | content 根节点要求 |
|------|---------|------------------|
| `doc.replace_page` | 1 (POPO文档) | **必须**单一 `<doc>` 根节点：`<doc><h1>...</h1><p>...</p></doc>` |
| `md.replace_page` | 4 (Markdown) | 无根节点要求，直接 Markdown 文本 |
| `doc.insert_after` / `doc.insert_before` / `doc.replace_node` | 1 | 不需 `<doc>` 包裹，content 为一个或多个块级标签 |

示例：

```json
{"type": "doc.replace_page", "content": "<doc><h1>标题</h1><p>正文</p><doc-li list-id=\"l1\" list-type=\"unordered\">项</doc-li></doc>"}
```
...
> ⚠️ **`=@file:` 仅对 `command` 类参数有效**。`popo_doc_create_doc` 的 `content` 参数是普通字符串，**不支持 `=@file:`**（误用 `content="@file:..."` 会把文件路径当字面内容写入文档）。多行/含特殊字符的 `content`（尤其 Windows）请用两步法：先 `doc_create_doc` 建空文档（不传 `content`），再用 `doc_update_doc` 的 `doc.replace_page`（docType=1）/ `md.replace_page`（docType=4）+ `=@file:` 写入内容。详见 [workflows.md 工作流 1 / 1b](./workflows.md)。

**Windows (PowerShell) — 必须使用 `=@file:` 临时文件，禁止直接内联：**

```powershell
# 1. 用 node 生成 JSON 临时文件（JSON.stringify 正确转义所有 Unicode 字符）
# ⚠️ 禁止用 PowerShell ConvertTo-Json：中文弯引号 "" 会被规范化成普通双引号导致坏 JSON
$genScript = @'
const fs = require("fs"); const path = require("path");
const cmd = { type: "doc.insert_after", content: "<h1>标题</h1><p>内容</p>" };
...
<attachment src="https://example.com/api/admin/file/download?path=abdwqsdwq&type=attachment&identity=sdwqweqwdwqxx" file-size="2048" />
...
- **图片嵌入文本容器** — 禁止放入 `h1`~`h6`、`p`、`doc-li`、`todo-item`；图片应独立写 `<img src="..." />`
